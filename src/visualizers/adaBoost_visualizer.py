...
class adaBoost_visualizer:
    ...
    def Visualise(self):
        ...
        print(list(self.grid.best_params_.values())[0])
        plt.figure(figsize=(10, 10))
        plt.plot(self.intervale, ymax, label='Best value is : ' + '{:1.3f}'.format(
            max(CVresults["mean_test_score"])) + ' for number of estimators = ' + '{:1.5f}'.format(
            list(self.grid.best_params_.values())[0]))
